...
class BinaryTreeLSTM_s(nn.Module):
    # Based on https://www.aclweb.org/anthology/P15-1150
    # TODO implement attention, dropout
    # maybe attention can be writen as a separate part?
...
class BinarySLSTMCell(nn.Module):
    ...
    def forward(self, l, r, inputs=None, dim=1):
        ...
        # input gate
        i_t = self.activate_func1(self.i(concat_input))
        # forget gate
        f_lt = self.activate_func1(self.f_l(concat_input))
        f_rt = self.activate_func1(self.f_r(concat_input))

        x_t = self.x(concat_h)
        # cell vector
        c_t = f_lt * c_l + f_rt * c_r + i_t * self.activate_func2(x_t)
        # output gate
        concat_tmp = torch.cat((h_l, h_r, c_t), dim=dim)
        o_t = self.activate_func1(self.o(concat_tmp))
        h_t = o_t * self.activate_func2(c_t)
...
class BUSLSTMCell(nn.Module):
    ...
    def forward(self, l, r, inputs=None, dim=0):
        ...
        z_t = self.activate_func1(self.z(child_x))
        x_t = z_t * x_l + (1.0 - z_t) * x_r
        x_in_t = self.x_in(x_t)
        x_i, x_f, x_o, x_g = x_in_t.chunk(4, dim=dim)
        concat_input = torch.cat((h_l, h_r, c_l, c_r), dim=dim)
        i_t = self.activate_func1(self.i(concat_input) + x_i)
        f_lt = self.activate_func1(self.f_l(concat_input) + x_f)
        f_rt = self.activate_func1(self.f_r(concat_input) + x_f)
        concat_g = torch.cat([h_l, h_r], dim=dim)
        g_t = self.activate_func2(x_g + self.g(concat_g))
        c_t = f_lt * c_l + f_rt * c_r + i_t * g_t
        concat_o = torch.cat([h_l, h_r, c_t], dim=dim)
        o_t = self.activate_func1(x_o + self.o(concat_o))
...
class TDLSTMCell(nn.Module):
    ...
    def forward(self, p, left, dim=0, root=False):
        ...
        if left:
            i_t = self.activate_func1(self.i_l(input_concat))
            f_t = self.activate_func1(self.f_l(input_concat))
            o_t = self.activate_func1(self.o_l(input_concat))
            g_t = self.activate_func2(self.g_l(g_concat))
        else:
            i_t = self.activate_func1(self.i_r(input_concat))
            f_t = self.activate_func1(self.f_r(input_concat))
            o_t = self.activate_func1(self.o_r(input_concat))
            g_t = self.activate_func2(self.g_r(g_concat))
