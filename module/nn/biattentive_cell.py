...
class LVeGBiattentiveClassificationNetwork(nn.Module):
    ...
    def __init__(self, embedder: TextFieldEmbedder,
                 ...
                 component_dim: int,
                 elmo: Elmo):
        super(LVeGBiattentiveClassificationNetwork, self).__init__()
        # pre_encode_feedforward
        ...
        if isinstance(output_dim, int):
            ...
            var_output_dim = [output_dim]
            mu_output_dim = [output_dim]
            output_dropout_prob = [output_dropout_prob]
        else:
            weight_output_dim = copy(output_dim)
            var_output_dim = copy(output_dim)
            mu_output_dim = copy(output_dim)
            output_layer_num = len(output_dim)
        weight_output_dim[-1] = weight_output_dim[-1] * component_dim
        mu_output_dim[-1] = mu_output_dim[-1] * component_dim * gaussian_dim
        var_output_dim[-1] = var_output_dim[-1] * component_dim * gaussian_dim
        self.weight_output_layer = Maxout(input_dim=integrtator_dim * 8, num_layers=output_layer_num,
                                          output_dims=weight_output_dim, pool_sizes=output_pool_size,
                                          dropout=output_dropout_prob)
        self.mu_output_layer = Maxout(input_dim=integrtator_dim * 8, num_layers=output_layer_num,
                                      output_dims=mu_output_dim, pool_sizes=output_pool_size,
                                      dropout=output_dropout_prob)

        self.var_output_layer = Maxout(input_dim=integrtator_dim * 8, num_layers=output_layer_num,
                                       output_dims=var_output_dim, pool_sizes=output_pool_size,
                                       dropout=output_dropout_prob)
...
class LABiattentiveClassificationNetwork(nn.Module):
    ...
    def __init__(self, embedder: TextFieldEmbedder,
        ...
        self.la_output_layer = Maxout(input_dim=integrtator_dim * 8, num_layers=output_layer_num,
                                      output_dims=la_output_dim, pool_sizes=output_pool_size,
                                      dropout=output_dropout_prob)
    ...
    def forward(self, tokens: Dict[str, torch.LongTensor]) -> Dict[str, torch.Tensor]:
        ...
        la = self.la_output_layer(pooled_representations_dropped)
